...
#     user: UserInDB = Relationship(back_populates="goal")
...
class GoalInDB(SQLModel, table=True):
    ...
    # type: Optional[str] = None # books, pages, or minutes
    period: Optional[str] = None # year, month, week, day
    amount: Optional[int] = None
...
class SessionInDB(SQLModel, table=True):
    ...
    # y_m_d: Optional[date] = Field(default_factory=date.today) # field value generated by database once saved
    # h_m_s: Optional[time] = Field(default_factory=lambda: datetime.now().time(), primary_key=True)
    prev_page: Optional[int] = 0
    cur_page: Optional[int] = 0
    minutes: Optional[int] = 0
    created_at: Optional[datetime] = Field(default_factory=datetime.now, primary_key=True)
